      ...
      <div class="agreementContent">
        <ul class="pingAgreeList clear">
          <li>
            ...
            <el-radio :disabled="disabled" v-model="httpObj.protocol" :label="'HTTP1.1'">HTTP1.1</el-radio>
          </li>
          <li>
            <span class="pingAgreelabel">超时时间（s)：</span>
            <el-input
...
export default {
  ...
  props: {
    /**
     * active { String} is used to inital protocolType
     *
     * disabled { Boolean } is used to judge showInfo or create
     * protocolConfig {Object} which is used to show protocolConfiguration
     * or initial protocolConfig
     */
    active: {
      type: String,
      default: "PING",
  ...
  watch: {
    ...
    disabled: function(newval, oldval) {
      ...
      let editors = document.querySelectorAll(".jsoneditor");
  ...
  methods: {
    /*
     * initial protocolConfig based on protocoltype
     *
     *
     */
    inittialProtocol() {
      switch (this.active) {
        case "PING":
          this.pingObj =
            this.isObjEmpty(this.protocolConfig) == "{}"
              ? this.pingObj
              : { ...this.protocolConfig };
          break;
        case "HTTP":
    ...
    /**
     * initial http jsonEditor based on status
     */
    initHttp() {
