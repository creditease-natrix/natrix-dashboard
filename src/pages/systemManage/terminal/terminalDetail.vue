    ...
    <div class="optionBox clear">
      ...
      <div id="primaryInfo" v-show="activeName == 'first' ">
        <ul class="primaryList">
          <li>
            <span class="listLabel">设备序列号：</span>
            <span>{{primaryInfo.sn}}</span>
          </li>
          <li>
            <span class="listLabel">主机名称：</span>
            <span>{{primaryInfo.hostname}}</span>
          </li>
          <li>
            <span class="listLabel">设备类型：</span>
            <span>{{primaryInfo.type}}</span>
          </li>
          <li>
            <span class="listLabel">操作系统：</span>
            <span>{{primaryInfo.os_type}}</span>
          </li>
          <li>
            <span class="listLabel">操作系统版本：</span>
            <span>{{primaryInfo.os_version}}</span>
          </li>
          <li>
            <span class="listLabel">客户端版本：</span>
            <span>{{primaryInfo.client_version}}</span>
          </li>
          <li>
            <span class="listLabel">状态：</span>
            <span>{{$t(primaryInfo.status)}}</span>
          </li>
          <li>
            <span class="listLabel">更新时间：</span>
            <span>{{primaryInfo.update_time}}</span>
          </li>
        </ul>
        <ul class="primaryList orgList">
          ...
          <li>
            <span class="listLabel">注释：</span>
            <span>{{primaryInfo.comment}}</span>
          </li>
        </ul>
      </div>
      <div id="systemInfo" v-show="activeName == 'second' ">
        <ul class="primaryList">
          ...
          <li>
            <span class="listLabel">操作系统名称：</span>
          ...
          <li>
            <span class="listLabel">操作系统内核版本：</span>
            <span>{{systemInfo.os_kernel_version}}</span>
          </li>
          <li>
            <span class="listLabel">架构信息：</span>
            <span>{{systemInfo.arch_info}}</span>
          </li>
          <li>
            <span class="listLabel">综合平台信息：</span>
            <span>{{systemInfo.platform}}</span>
          </li>
          <li>
            <span class="listLabel">python版本：</span>
            <span>{{systemInfo.python_version}}</span>
          </li>
          <li>
            <span class="listLabel">natrix版本：</span>
            <span>{{systemInfo.natrix_version}}</span>
          </li>
          <li>
            <span class="listLabel">chrome版本：</span>
            <span>{{systemInfo.chrome_version}}</span>
          </li>
          <li>
            <span class="listLabel">chrome webdriver版本：</span>
            <span>{{systemInfo.chrome_version}}</span>
          </li>
          <li>
            <span class="listLabel">firefox版本：</span>
            <span>{{systemInfo.firefox_version}}</span>
          </li>
          <li>
            <span class="listLabel">firefox webdriver版本：</span>
            <span>{{systemInfo.firefox_webdriver_version}}</span>
          </li>
        </ul>
      </div>
      <div id="deviceInfo" v-show="activeName == 'third' ">
        <ul class="primaryList">
          <li>
            <span class="listLabel">设备序号：</span>
            <span>{{deviceInfo.sn}}</span>
          </li>
          <li>
            <span class="listLabel">启动时间：</span>
            <span>{{deviceInfo.boot_time}}</span>
          </li>
          <li>
            <span class="listLabel">产品信息：</span>
            <span>{{deviceInfo.product}}</span>
          </li>
          <li>
            <span class="listLabel">CPU型号：</span>
            <span>{{deviceInfo.cpu_info.cpu_model}}</span>
          </li>
          <li>
            <span class="listLabel">CPU使用率（%）：</span>
            <span>{{deviceInfo.cpu_info.cpu_percent}}</span>
          </li>
          <li>
            <span class="listLabel">内存信息：</span>
            <span>{{deviceInfo.memory_info.memory_total}}</span>
          </li>
          <li>
            <span class="listLabel">内存使用率（%）：</span>
            <span>{{deviceInfo.memory_info.memory_percent}}</span>
          </li>
          <li>
            <span class="listLabel">磁盘使用率（%）：</span>
            <span>{{deviceInfo.disk_info.disk_percent}}</span>
          </li>
        </ul>
      </div>
      <div id="netInfo" v-show="activeName == 'fourth' ">
        <el-collapse v-model="activeNames" v-for="(item,index) in netInfo" :key="index">
          <el-collapse-item :title="item.name+'-'+item.type" :name="index">
            <ul class="primaryList">
              <li>
                <span class="listLabel">本地IP地址：</span>
                <span>{{item.localip}}</span>
              </li>
              <li>
                <span class="listLabel">子网掩码：</span>
                <span>{{item.netmask}}</span>
              </li>
              <li>
                <span class="listLabel">网关地址：</span>
                <span>{{item.gateway}}</span>
              </li>
              <li>
                <span class="listLabel">MAC地址：</span>
                <span>{{item.macaddress}}</span>
              </li>
              <li>
                <span class="listLabel">公网地址：</span>
                <span>{{item.publicip}}</span>
              </li>
              <li>
                <span class="listLabel">地域信息：</span>
                <span>{{item.location_info.country +'-' +item.location_info.province +'-' +item.location_info.city}}</span>
              </li>
              <li style="width:70%;">
                <span class="listLabel">状态：</span>
                <span class="listOption">
                  <span>
                    <i
                      class="icon iconfont icon-kaiqi"
                      :class="{success:item.is_active,error:!item.is_active}"
                    ></i>开启
                  </span>
                  <span>
                    <i
                      class="icon iconfont icon-queshengye"
                      :class="{success:item.is_default,error:!item.is_default}"
                    ></i>缺省网卡
                  </span>
                  <span>
                    <i
                      class="icon iconfont icon-wuxianjuyuwang"
                      :class="{success:item.access_intranet,error:!item.access_intranet}"
                    ></i>局域网访问
                  </span>
                  <span>
                    <i
                      class="icon iconfont icon-neiwang"
                      :class="{success:item.access_corporate,error:!item.access_corporate}"
                    ></i>企业网访问
                  </span>
                  <span>
                    <i
                      class="icon iconfont icon-gongsiwangdian"
                      :class="{success:item.access_internet,error:!item.access_internet}"
                    ></i>互联网访问
                  </span>
                </span>
              </li>
      ...
      <div id="alive" v-show="activeName == 'fifth' ">
        <el-table :data="tableData" border style="width: 100%,margin-top:50px;">
          <el-table-column prop="receive_time" label="接收时间" width="200"></el-table-column>
          <el-table-column prop="send_time" label="发送时间" width="200"></el-table-column>
          <el-table-column prop="type" label="信息类型"></el-table-column>
        </el-table>
        <page :pageCount="page_Count" :item_count="item_count" v-on:page-change="pageChangeHandle" v-show="pageShow"></page>
      </div>
    </div>
  </div>
</template>
...
<script>
import { timeStamp1, utc2beijing ,messageTip} from "../../../until/index.js";
...
export default {
  ...
  methods: {
    ...
    pageChangeHandle(currentPageNum) {
        this.num =  currentPageNum //获取当前页码
...
#netInfo .el-collapse-item__wrap{
  padding-left:30px;
}
#netInfo .primaryList{
  padding-left:0;
