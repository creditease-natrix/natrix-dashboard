<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="name" label="登记职场">
        <template slot-scope="scope">
          <el-popover
            trigger="hover"
            placement="top"
      ...
      <el-table-column prop="type" label="检测职场">
        <template slot-scope="scope">
          <el-popover
            trigger="hover"
            placement="top"
            v-for="(item1,index1) in scope.row.detect_orgs"
            :key="index1"
          >
            <p>名称: {{ item1.desc }}</p>
            <div slot="reference" class="name-wrapper">
      ...
      <el-table-column prop="terminal_name" width="120" label="终端名称">
        <!-- <template slot-scope="scope">
      ...
      <el-table-column prop="terminal_local_ip" width="120" label="IP地址">
        <!-- <template slot-scope="scope">
                    <ul class="terminalList">
                        <li v-for="(item,index) in scope.row.terminals" :key="index" class="terminal">
                            {{item.local_ip}}
      ...
      <el-table-column prop="terminal_status" width="120" label="终端状态">
        <template slot-scope="scope">
          <span
            class="terminalStatus"
            :class="{success:scope.row.terminal_is_active,error:!scope.row.terminal_is_active }"
          >{{scope.row.terminal_status}}</span>
        </template>
      </el-table-column>
    ...
    <page :pageCount="page_Count" v-on:page-change="pageChangeHandle" v-show="pageShow"></page>
  </div>
</template>
